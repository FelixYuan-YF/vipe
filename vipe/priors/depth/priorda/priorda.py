...
class PriorDepthAnything(nn.Module):
    def __init__(
        ...
        if fmde_dir is None:
            fmde_path = hf_hub_download(repo_id=self.args.repo_name, filename=fmde_name, local_dir="checkpoints/prior_depth_anything")
        else:
        ...
        if not coarse_only:
            ...
            if cmde_dir is None:
                cmde_path = hf_hub_download(repo_id=self.args.repo_name, filename=cmde_name, local_dir="checkpoints/prior_depth_anything")
            else:
    ...
    def load_checkpoints(self, model, ckpt_dir, device="cuda:0"):
        ...
        if ckpt_dir is None:
            ckpt_path = hf_hub_download(repo_id=self.args.repo_name, filename=ckpt_name, local_dir="checkpoints/prior_depth_anything")
        else:
